...
export type PageControlBarElement = HTMLDivElement;
export type PageControlBarProps = React.ComponentProps<'div'>;
function PageControlBar({
    className,
    children,
    ...other
}: PageControlBarProps): React.JSX.Element {
    return (
        <div
            className={cn(
                'relative flex h-14 flex-row items-center gap-1 border-t p-2',
                className
            )}
            {...other}
        >
