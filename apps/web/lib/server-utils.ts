...
export const getSafeAuthSession = cache(
    async (options: GetSafeAuthSessionOptions = {}) => {
        ...
        if (options.requiredPermission) {
            ...
            if (!resource || !action) {
                throw new Error(
                    `Invalid permission format. Expected "resource:action", got: ${options.requiredPermission}`
                )
            }
